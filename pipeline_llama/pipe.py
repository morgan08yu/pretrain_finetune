...
class Pipe4LlamaModelCasualLM:
    ...
    def create_pipe_model(self, on_cpu: bool = True) -> nn.Sequential:
        ...
        for index in range(self.base_config.num_hidden_layers):
            target_gpu = last_gpu if index < 2 else first_gpu
...
#     def __init__(self, total_sample: int = 10000, batch_size: int = 2, seq_length: int = 1024) -> None:
...
class PipeTrain:
    ...
    def train(self, total_epoch: int = 10000, dataset =  None):
        ...
        input_ids, labels = dataset.input_ids, dataset.labels
...
def train(ngpus: int = 2):
    init_rpc()
    # base_config = AutoConfig.from_pretrained(path, torch_dtype=torch.bfloat16)
    # base_model = LlamaForCausalLM(base_config)
    ...
    plc = Pipe4LlamaModelCasualLM(
        config= base_model.config, transformersmodel=base_model, ngpus=ngpus)
    ...
    # tokenizer = AutoTokenizer.from_pretrained("/home/devsrc/LLMs/meta-llama3-8b", use_fast=False)
    data = GenerateTraindata()
    # lm_datasets = TrainDataset(args=, tokenizer=tokenizer).build_dataset()
    pipetrain = PipeTrain(model=pipe_model_pytorch, config=config)
    pipetrain.train(total_epoch=1000, dataset=data)
